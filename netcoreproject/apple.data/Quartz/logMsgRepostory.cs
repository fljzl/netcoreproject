...
namespace apple.data.Quartz
{
    public class logMsgRepostory : SqlSugarDBContent
    {
        ...
        {
            return Db.Queryable<logMsg>().Where(d => d.JobName.Contains(content)).OrderBy(it => it.CreateTime, SqlSugar.OrderByType.Desc).ToPageList(pageIndex, pageSize, ref totals);
        }
    }
}
